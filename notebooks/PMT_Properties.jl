...
# ╔═╡ 718c1dca-10eb-11ef-2e11-f16d42427cff
begin
    using CairoMakie
...
# ╔═╡ de0c6850-7069-4a86-9e92-781c2a016ede
pkgversion(LumenManufaktur)

# ╔═╡ 346b8617-0c1c-484d-8764-d6815c9598ff
pmt = LumenManufaktur.KM3NeTPMT

# ╔═╡ f71d1577-b230-48a9-907f-322dfa115cfc
λs_data = range(270.0, 710; step = 5.0)

# ╔═╡ e987254d-1950-4403-8c72-090ed37c383a
#λs = range(270.0, 710)
λs = range(270.0, 710)

# ╔═╡ adf7446d-7ada-4179-91b4-db4e8c8395f8
QEs = map(pmt.quantum_efficiency, λs);

# ╔═╡ 09bbf94c-89a5-48f8-a8d2-c27434ae165e
let
    for (λ, QE) in zip(λs, QEs)
        println("$λ $QE")
    end
end

# ╔═╡ 77be626f-7d3d-45f8-ba17-bcb1c2c463ea
JPP_QE = CSV.File("KM3NeT_QE.csv") |> DataFrame;

# ╔═╡ 54f402fe-4f6d-452e-a7b0-ad54506b141a
let
    for (λ, QE) in zip(JPP_QE.lambda, JPP_QE.QE)
        println("$λ $QE")
    end
end

# ╔═╡ cc40316d-49f2-45d7-8de6-0854e29e3dc2
let
    fig = Figure(size = (600, 400))
    ax = Axis(
        fig[1, 1],
        xlabel = "photon wavelength / ns",
        ylabel = "QE",
        xgridstyle = :dash,
        ygridstyle = :dash,
    )
    #plot!(ax, λs_data, map(pmt.quantum_efficiency, λs_data), label="Data")
    plot!(ax, λs, QEs, label = "LumenManufaktur.jl")
    plot!(ax, JPP_QE.lambda, JPP_QE.QE, label = "Jpp")
    fig
end

# ╔═╡ 65769389-48c7-4d44-a452-3e647f58b835
...
LumenManufaktur = "a63c60df-84c5-40dc-bdf1-cfe657ef319c"
...
manifest_format = "2.0"
project_hash = "5218f118112080e91eaee4cc263f11da2ff42303"
...
[[deps.LumenManufaktur]]
deps = ["BasicInterpolators", "FastGaussQuadrature"]
git-tree-sha1 = "c1baec55aaf25344baa10e7870da4360f683c7c2"
